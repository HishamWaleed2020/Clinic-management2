...
function getGender222() {
	...
	exit;
	$data = '<option value="">اختار الجنس</option>';

	$data = $data .'<option value="Male">ذكر</option>';
	$data = $data .'<option value="Female">انثئ</option>';
...
function getMedicines($con, $medicineId = 0) {
	...
	$data = '<option value="">اختار الدواء</option>';
...
from `patients` order by `patient_name` asc;";
	...
	$data = '<option value="">اختار المريض</option>';
